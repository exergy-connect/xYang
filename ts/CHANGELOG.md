...
- **`metadata-substatements` helpers:** `withMetadataSubstatements` / `withDataNodeSubstatements` for shared `description`, `reference`, and ignored `config` handlers.
- **Tests:** `test/parser_parity.test.ts` covering the above (typedef default, pattern concat, implicit choice, config warning, syntax error format, multi-predicate paths, include-path imports, leaf `reference`).

### Changed

- **`config` substatement:** consumed with a console warning on data definition nodes; not stored on the AST.
